...
try {
    ...
    Assert-Equal -Expected "2.0.2" -Actual ([string]$manifest.version) -Message "Manifest package version is incorrect."
    ...
    Assert-Throws -Action {
        & $bundleScript -Tag "release/mz.ConfigAPI.Consumer/2.0.2" -OutputDirectory (Join-Path $testRoot "wrong-case") -SkipTests | Out-Null
